CLASS zcl_ton_homescreen_cntrl DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC

  GLOBAL FRIENDS zcl_ton_customer_login_cntrl .

  PUBLIC SECTION.

    CONSTANTS lc_max_number_of_product TYPE i VALUE 50 ##NO_TEXT.
    DATA: mo_login_cntrl      TYPE REF TO zcl_ton_customer_login_cntrl,
          mo_home_screen_view TYPE REF TO zcl_ton_homescreen_view,
          mt_order_to_show    TYPE zton_tt_order,
    ...
    METHODS: add_buttons_show_position
      ...
      search_selected_product
        RETURNING
          VALUE(rs_selected_product) TYPE zton_article ,
      on_pbo_cart ,
      on_back ,
      on_leave
        RAISING zcx_ton_webshop_exception_new ,
      ...
      constructor
        IMPORTING
          !io_login_cntrl TYPE REF TO zcl_ton_customer_login_cntrl
  ...
  PRIVATE SECTION.
    ...
    DATA:
      mo_home_screen_model          TYPE REF TO zcl_ton_homescreen_model,
    ...
    METHODS:
      ...
      search_selected_item_in_cart
        RETURNING
          VALUE(rs_selected_item) TYPE zton_s_cart ,
...
CLASS ZCL_TON_HOMESCREEN_CNTRL IMPLEMENTATION.
  ...
  METHOD create_alv_for_cart.
    ...
    TRY.
        ...
        me->mo_alv_grid_cart->set_table_for_first_display( EXPORTING  i_structure_name = 'ZTON_S_CART'
        ...
        IF sy-subrc <> 0.
          MESSAGE i052(zton_web_shop) WITH sy-subrc INTO DATA(ls_msg).
  ...
  METHOD create_alv_for_home_screen.
    ...
    TRY.
        ...
        IF me->mo_container_home_screen IS BOUND AND me->mo_alv_grid_home_screen IS NOT BOUND.
          ...
          me->mo_alv_grid_home_screen->set_table_for_first_display( EXPORTING i_structure_name = 'ZTON_ARTICLE'
          ...
          IF sy-subrc <> 0.
            MESSAGE i053(zton_web_shop) WITH sy-subrc INTO DATA(ls_msg).
            me->mo_log->add_msg_from_sys( ).
            me->mo_log->safe_log( ).
            RAISE EXCEPTION TYPE zcx_ton_webshop_exception_new USING MESSAGE.
          ENDIF.
...
*          SET HANDLER me->handle_node_context_menu_sel FOR me->mo_tree.
  ...
  METHOD delete_node_and_items.
    ...
    me->mo_tree->delete_node( EXPORTING  node_key = CONV #( is_position-order_number ) " Node key
                              EXCEPTIONS failed            = 1                " General Error
                                         ...
                                         OTHERS            = 4  ).
    IF sy-subrc <> 0.
      MESSAGE i062(zton_web_shop) WITH sy-subrc.
      me->mo_log->add_msg_from_sys( ).
      me->mo_log->safe_log( ).
    ENDIF.
  ...
  METHOD edit_quantity_order_position.
    ...
    TRY.
        ...
        IF lt_index_row IS INITIAL.
          MESSAGE i067(zton_web_shop) INTO DATA(lv_msg).
          me->mo_log->add_msg_from_sys( ).
          me->mo_log->safe_log( ).
          RAISE EXCEPTION TYPE zcx_ton_webshop_exception_new USING MESSAGE.
        ENDIF.
        ...
        IF lv_quantity IS INITIAL OR lv_quantity <= 0.
          MESSAGE i066(zton_web_shop) INTO lv_msg.
          me->mo_log->add_msg_from_sys( ).
          me->mo_log->safe_log( ).
          RAISE EXCEPTION TYPE zcx_ton_webshop_exception_new USING MESSAGE.
        ENDIF.
  ...
  METHOD on_add_product_to_cart.
    ...
    TRY.
        ...
        ELSEIF lv_quantity <= 0.
          MESSAGE i048(zton_web_shop) INTO DATA(ls_msg).
          me->mo_log->add_msg_from_sys( ).
          me->mo_log->safe_log( ).
          RAISE EXCEPTION TYPE zcx_ton_webshop_exception_new USING MESSAGE.

        ELSEIF lv_quantity >= lc_max_number_of_product.
          MESSAGE i049(zton_web_shop) INTO ls_msg.
          me->mo_log->add_msg_from_sys( ).
          me->mo_log->safe_log( ).
          RAISE EXCEPTION TYPE zcx_ton_webshop_exception_new USING MESSAGE.
        ENDIF.
  ...
  METHOD on_edit_quantity.
    ...
    TRY.
        ...
        IF lv_quantity > 0 AND lv_quantity < lc_max_number_of_product.
          ...
          DATA ls_item_for_cart TYPE zton_article.
          DATA(ls_item) =  me->search_selected_item_in_cart( ).

          ls_item_for_cart = VALUE zton_article( article_number = ls_item-article_number
  ...
  METHOD on_order.
    ...
    TRY.
        ...
        ELSE.
          MESSAGE i050(zton_web_shop) INTO DATA(ls_msg).
          RAISE EXCEPTION TYPE zcx_ton_webshop_exception_new USING MESSAGE.
        ENDIF.
      CATCH zcx_ton_webshop_exception_new INTO DATA(e_text).
        MESSAGE e_text->get_text( ) TYPE 'S' DISPLAY LIKE 'E'.
  ...
  METHOD on_remove_item_from_cart.
    ...
    TRY.
        ...
        IF me->mo_home_screen_model->mt_cart IS INITIAL.
          MESSAGE i051(zton_web_shop) INTO DATA(ls_msg).
          me->mo_log->add_msg_from_sys( ).
          me->mo_log->safe_log( ).
          RAISE EXCEPTION TYPE zcx_ton_webshop_exception_new USING MESSAGE.
        ENDIF.
        ...
        me->mo_alv_grid_cart->refresh_table_display( ).
        IF sy-subrc <> 0.
          MESSAGE i047(zton_web_shop) INTO ls_msg.
          me->mo_log->add_msg_from_sys( ).
          me->mo_log->safe_log( ).
          RAISE EXCEPTION TYPE zcx_ton_webshop_exception_new.
        ENDIF.
  ...
  METHOD on_show_cart.
    ...
    IF me->mo_home_screen_model->mt_cart IS NOT INITIAL.
      ...
      IF mo_cart_view IS NOT BOUND.
        mo_cart_view = NEW zcl_ton_cart_view( io_cntrl = me ).
      ENDIF.
  ...
  METHOD search_selected_product.
    ...
    TRY.
        ...
        IF ls_product IS INITIAL.
          MESSAGE i043(zton_web_shop) INTO DATA(ls_msg).
          me->mo_log->add_msg_from_sys( ).
          me->mo_log->safe_log( ).
          RAISE EXCEPTION TYPE zcx_ton_webshop_exception_new USING MESSAGE.
        ENDIF.
  ...
  METHOD show_article.
    ...
    IF mo_custom_container_article IS NOT BOUND.
      ...
      mo_alv_grid_article->set_table_for_first_display( EXPORTING i_structure_name = 'ZTON_ARTICLE'
    ...
    TRY.
        ...
        CALL FUNCTION 'DP_PUBLISH_WWW_URL'
          ...
          EXCEPTIONS
            ...
            OTHERS                = 4.
        IF sy-subrc <> 0.
          MESSAGE: i063(zton_web_shop) WITH sy-subrc INTO DATA(ls_msg).
          me->mo_log->add_msg_from_sys( ).
          me->mo_log->safe_log( ).
          RAISE EXCEPTION TYPE zcx_ton_webshop_exception_new USING MESSAGE.
        ENDIF.
        ...
        me->mo_logo_home_screen->load_picture_from_url_async( EXPORTING    url    = lv_url
                                                              EXCEPTIONS   error  = 1
                                                                           OTHERS = 2 ).
        IF sy-subrc <> 0.
          MESSAGE: i064(zton_web_shop) WITH sy-subrc INTO ls_msg.
          me->mo_log->add_msg_from_sys( ).
          me->mo_log->safe_log( ).
          RAISE EXCEPTION TYPE zcx_ton_webshop_exception_new USING MESSAGE.
        ENDIF.
        ...
        me->mo_logo_home_screen->set_display_mode( EXPORTING   display_mode = cl_gui_picture=>display_mode_fit
                                                   EXCEPTIONS  error        = 1
                                                               OTHERS       = 2 ).
        IF sy-subrc <> 0.
          MESSAGE: i065(zton_web_shop) WITH sy-subrc INTO ls_msg.
          me->mo_log->add_msg_from_sys( ).
          me->mo_log->safe_log( ).
          RAISE EXCEPTION TYPE zcx_ton_webshop_exception_new USING MESSAGE.
        ENDIF.
  ...
  METHOD show_order.
    ...
    IF it_order IS NOT INITIAL.
      IF me->mo_container_show_position IS NOT BOUND.
        ...
        me->mo_alv_grid_show_position->set_table_for_first_display( EXPORTING i_structure_name = 'ZTON_ORDER'
